  ...
  return (
    <section className="section container payments-center-new">
      ...
      {!filteredPayments.length ? (
        ...
        <div className="order-shelf payments-grid-sq">
          {filteredPayments.map((order) => {
            const status = String(order.paymentStatus || "").toLowerCase();
            const isPaid = status === 'paid';
            
            return (
              <div className="shelf-item" key={order.id}>
                <OrderCard
                  order={order}
                  apiBase={apiBase}
                  onDetails={() => openDetails(order.id)}
                  onPay={!isPaid && String(order.status || "").toLowerCase() !== "cancelled" ? () => beginPaymentForOrder(order.id) : null}
                  // We show payment status in the card context if needed, 
                  // but OrderCard already shows Order Status which matches user screenshot.
                />
              </div>
            );
          })}
